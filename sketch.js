...
function draw(){
   ...
   if(gamest==0){
    ...
    if(gamest==1){
        ...
        Box1=createSprite(displayWidth/2-60,displayHeight/10,80,80);
        Box1.visible=false;

        Box2=createSprite(displayWidth/2+60,displayHeight/10,80,80);
        Box2.visible=false;
     ...
     for(var i1=0;i1<leftgroup.length;i1++){
        if(mousePressedOver(leftgroup.get(i1))){
           leftgroup.get(i1).x=Box1.x
           leftgroup.get(i1).y=Box1.y;
    ...
    for(var i2=0;i2<rightgroup.length;i2++){
        if(mousePressedOver(rightgroup.get(i2))){
            rightgroup.get(i2).x=Box2.x;
            rightgroup.get(i2).y=Box2.y;  
                
}   
    
    
  
if(leftgroup.get(i1).isTouching(Box1)&&rightgroup.get(i2).isTouching(Box2)){
  textSize(30);
  text ("+",displayWidth/2-10,displayHeight/10);
}

if((alum.isTouching(Box1)&&oxygen2.isTouching(Box2))||alum2.isTouching(Box2)&&oxygen.isTouching(Box1)){
 // text("2Al + 3O  ====>Al O",displayWidth/2,displayHeight/2);
 // text("                 2 3",displayWidth/2,displayHeight/2+20);
...
/*if(gamest==2){
   ...
   d.visible=false;
    ...
    H.addImage("Hydrogen",Hy);
    H.scale=0.8;
    
    if(mousePressedOver(H)){
     d.addImage("Hydrogen",Hd)
     Lit.visible=false;
      d.visible=true;
  console.log("Hey");
